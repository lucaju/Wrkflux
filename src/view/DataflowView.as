package view {
	...
	public class DataflowView extends AbstractView {
		...
		public function init(stepCollection:Array):void {
			...
			this.addEventListener(WrkfluxEvent.SELECT, clickPin);
		}
		...
		private function generatePins(dataCollection:Array, stepCollecion):void {
			...
			for each(var doc:DocumentModel in dataCollection) {
				...
				//TweenMax.from(pinView,1,{alpha:0, scaleX:0, scaleY:0, delay:2+(i*.015), ease:Back.easeOut, onStart: WorkflowController(this.getController()).addPinToStep, onStartParams: [pinView.id, sp.id]});
				
				//TweenMax.from(pinView,.1,{y:-50, delay:i*.01, onStart: WorkflowController(this.getController()).addPinToStep, onStartParams: [pinView.id, sp.id]});
