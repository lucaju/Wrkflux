package view {
	...
	import view.workflow.list.PinListItemItem;
	...
	public class WorkflowView extends AbstractView {
		...
		protected var pinList						:PinListView;			
		...
		private function initiate():void {
			...
			//8. listenter
			wController.getModel("data").addEventListener(WrkfluxEvent.UPDATE_PIN, updatePin);
			...
			topBar.addEventListener(WrkfluxEvent.SELECT, topBarAction);
			
			dataflowView.addEventListener(WrkfluxEvent.DRAG_PIN, hitTest);
			dataflowView.addEventListener(WrkfluxEvent.SELECT, clickPin);
		...
		protected function topBarAction(event:WrkfluxEvent):void {
			...
			switch (event.data.label) {
				case "list":
					...
					if (event.data.toggle) {
						...
						pinList = new PinListView(this.getController());
						pinList.y = topBar.height;
						this.addChildAt(pinList,2);
						pinList.init(WorkflowController(this.getController()).getPinsData(), this.stage.stageHeight - topBar.height);
						pinList.addEventListener(WrkfluxEvent.SELECT, pinListSelect);
						...
						pinList.removeEventListener(WrkfluxEvent.SELECT, pinListSelect);
		 ...
		 * 
		 */
		protected function clickPin(event:WrkfluxEvent):void {
		 ...
		 * 
		 */
		protected function pinListSelect(event:WrkfluxEvent):void {
			var pin:PinView = dataflowView.getPinById(event.data.id);
		 ...
		 * 
		 */
		protected function updatePin(event:WrkfluxEvent):void {
